...
# req = requests.get('https://parsinger.ru/4.3/4/index.html')
# req.encoding = 'utf-8'
#
...
#     p = soup.find_all('p')
#     for el in p:
#         if len(el.text.replace(" ", "")) % 2 == 0:
#             id = int(el.get('id')[0])
#             class_m = int(el.get('class')[0])
#             f = id + class_m
#             sum+=f
#     print(sum)
...
def sum_even_length_ids(html):
    ...
    for el in p:
        if len(el.text.replace(" ", "")) % 2 == 0:
            total_id_sum += int(el.get('id'))
            total_class_sum += int(el.get('class')[0])
