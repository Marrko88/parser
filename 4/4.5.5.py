...
def calculate_total_price(html: str) -> float:
    ...
    # Ваш код
    book_card = soup.find_all('div', {'class': 'book-card'})

    for el in book_card:
        price = el.find('p', class_ = 'count price').text
        stock = el.find('p', class_ = 'count stock').text
        total_price = float(price[7:]) * float(stock[22:])
